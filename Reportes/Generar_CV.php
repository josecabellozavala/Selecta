<?php
	...
	*/
	...
	$pdf->Cell(0,10, '1  Datos Generales.', 0, 'L');
	$pdf->Ln();
   ...
   // * TODOS LOS PUNTOS DEL 2
	...
	for($arti = 0; $arti < 33; $arti++)
	{
		...
		{   
			...
			{  
				...
				$pdf->SetFont('Arial', '', 14);
				$pdf->Multicell(0,5,"2.6  ".utf8_decode($result3[$parasoloundato]["Descripcion"]), 0, 'L');            
				$pdf->Ln();
				for($x = 0; $x < count($result); $x++)
				{                                        
	...
	//...3.2       
	$bandera2 = false; 
	$result = $conexion->Consultas("SELECT FK_Tesis, Etiqueta_Copei, FK_Tipo, Titulo, Concluida, Fecha_Final, Lugar  FROM Usuario_Tesis, Tesis WHERE ID_Tesis = FK_Tesis AND Tesis.Fecha_Final < '".$fecha_Final[2]."-".$fecha_Final[1]."-".$fecha_Final[0]."' AND  FK_Usuario = ".$id_usuario." ORDER BY FK_Tipo;");        
	if(count($result) > 0)
	{         
		$bandera3 = false;
		$bandera4 = false;
	...
	{                 
		...
		if($bandera == false)
		{
			$pdf->Multicell(0,5,utf8_decode("4  Repercusi�n Acad�mica."), 0, 'L');
			$pdf->Ln();
	...
	{                 
		...
		$pdf->SetFont('Arial', '', 14);
		$pdf->Multicell(0,5,"4.2  ".utf8_decode($result2[$parasoloundato]["Descripcion"]), 0, 'L');            
		$pdf->Ln();                                                                                   
		$pdf->SetFont('Arial', '', 10);
		for($x = 0; $x < count($result); $x++)
		{   
	...
	{ 
		$pdf->SetFont('Arial', '', 14);
		$pdf->Multicell(0,5,utf8_decode("5  Criterios Adicionales."), 0, 'L');
		$pdf->Ln();
		$bandera = true;                                                                                
		for($x = 0; $x < count($result); $x++)
		{ 
